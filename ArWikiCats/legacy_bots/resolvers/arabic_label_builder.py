...
This module handles the construction of Arabic labels from category strings.
It is extracted from the legacy circular_dependency package to break the import cycle.

This module imports from country_resolver but NOT the other way around,
establishing a proper DAG of imports.
"""
...
from .country_resolver import event2_d2, fetch_country_term_label
...
class Fixing:
    ...
    def determine_separator(self) -> str:
        ...
        This method selects the appropriate Arabic separator based on the parsed English separator
        and internal mappings. It may append the preposition " في" to self.type_label when applicable
        and remove the original English separator from self.cate_test when a mapped Arabic separator
        is applied. The method also consults translation_category_relations and other heuristics to
        handle special cases (e.g., "for", "to") and returns the finalized separator string including
        surrounding spaces when non-empty.

        Returns:
            ar_separator (str): The Arabic separator to join type and country labels (may be a single
                                space or a spaced Arabic preposition).
        """
        ar_separator = " "
        ...
        if self.add_in_lab:
            ...
            if separator2_lab not in separators_lists_raw:
                tatl = separator2_lab
                logger.info(
                    f">>>>> > ({self.separator_stripped=}): separator_stripped in category_relation_mapping "
                    f"and separator_stripped not in separators_lists_raw, {tatl=}"
                )
...
class LabelPipeline(Fixing):
    ...
    def resolve_labels(self) -> bool:
        ...
        if self.type_label or self.country_label:
            logger.info(
                f'<<lightgreen>>>>>> ------------- country_lower:"{self.country_lower}", '
                f'country_label:"{self.country_label}"'
            )
            logger.info(
...
def find_ar_label(
    ...
    This function uses the LabelPipeline class to perform the logic.
    """
