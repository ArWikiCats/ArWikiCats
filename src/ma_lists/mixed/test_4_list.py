...
def _extend_female_sport_mappings() -> None:
    ...
    for key, value in sport_formts_female_nat.items():
        en_is_nat_ar_is_al_women[key] = value
    for category, label in baston_women.items():
        en_is_nat_ar_is_al_women[category.lower()] = f"{label} {{}}"
...
def _extend_singer_and_business_entries() -> None:
    ...
    for key, label in businesspeoples.items():
        en_is_nat_ar_is_women[f"{key} businesspeople"] = f"شخصيات أعمال {{}} في {label}"
        en_is_nat_ar_is_women[
            f"{key} industry businesspeople"
        ] = f"شخصيات أعمال {{}} في صناعة {label}"
...
def _extend_book_entries() -> None:
    ...
    for key, label in Books_table.items():
        lowered = key.lower()
        en_is_nat_ar_is_women[lowered] = f"{label} {{}}"
        for book_type, book_label in Books_type.items():
...
Years_List: Final[list[int]] = [13, 14, 15, 16, 17, 18, 19, 20, 21, 23, 24]
