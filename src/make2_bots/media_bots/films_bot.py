...
from ...helps.print_bot import print_def_head, output_test, main_output_preferences
...
def test_films(category: str, reference_category: str = "") -> str:
    ...
    if normalized_category in RESOLVED_CATEGORY_CACHE:
        ...
        output_test(
            f'>>>> normalized_category: "{normalized_category}" already resolved, label:"{cached_label}"'
        )
        return cached_label

    print_def_head(
        f"<<lightblue>>>> xxxxxxxxxx test_films normalized_category:{normalized_category} xxxxxxxxxxx "
    )
    resolved_label = ""
    ...
    if not resolved_label:
        ...
        if resolved_label:
            ...
            output_test(
                f'>>>> Jobs_in_Multi Sports: New_players[{normalized_category}] ="{resolved_label}"'
            )

    if not resolved_label:
        resolved_label = test4_2018_with_nat(
            normalized_category, out=main_output_preferences[1], reference_category=reference_category
        )
        if resolved_label:
            Add_to_main2_tab(normalized_category, resolved_label)
    ...
    if not resolved_label:
        ...
        if resolved_label:
            ...
            Add_to_main2_tab(normalized_category, resolved_label)
            output_test(
                f'>>>> test_4 2018 Jobs: New_players[{normalized_category}] ="{resolved_label}"'
            )
    ...
    print_def_head(
        f"<<lightblue>>>> xxxxxxxxx test_films end xxxxxxxxxxx resolved_label:{resolved_label}"
    )
