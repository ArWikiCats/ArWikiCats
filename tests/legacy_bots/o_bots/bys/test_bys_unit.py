"""Tests for :mod:`legacy_bots.legacy_resolvers.bys`."""

from __future__ import annotations

import pytest

from ArWikiCats.legacy_bots.legacy_resolvers import bys


@pytest.mark.unit
def test_make_by_label_prefers_film_labels(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "ArWikiCats.legacy_bots.legacy_resolvers.bys.all_new_resolvers",
...
def test_find_dual_by_keys_supports_dual_categories(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "ArWikiCats.legacy_bots.legacy_resolvers.bys.resolve_by_labels",
...
def test_get_by_label_combines_entity_and_suffix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "ArWikiCats.legacy_bots.legacy_resolvers.bys.get_from_new_p17_final",
