...
def test_all_seasons_parentheses(i):
...
@pytest.mark.fast
@pytest.mark.parametrize("i", range(1, 11))
def test_all_seasons_no_parentheses(i):
    text = f"My Show season {i} episodes"
    list_of_cat, category3 = get_episodes(text)
    assert list_of_cat == f"حلقات {{}} الموسم {i}"
    assert category3 == "My Show"
...
@pytest.mark.fast
def test_override_category3_nolower():
...
@pytest.mark.fast
def test_does_not_trim_middle_text():
