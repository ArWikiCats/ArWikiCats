...
# --- keys_to_pattern -------------------------------------------------
def test_keys_to_pattern_returns_pattern(sample_data) -> None:
...
def test_match_key(category, expected, sample_data) -> None:
    formatted_data, data_list = sample_data
    bot = FormatData(formatted_data, data_list)
...
def test_apply_pattern_replacement(template, sport, expected, sample_data) -> None:
...
def test_normalize_category(category, sport_key, expected, sample_data) -> None:
    bot = FormatData(*sample_data)
    assert bot.normalize_category(category, sport_key).lower() == expected.lower()


# --- get_template ----------------------------------------------
def test_get_template_found(sample_data) -> None:
    formatted_data, data_list = sample_data
    bot = FormatData(formatted_data, data_list)
...
def test_get_template_not_found(sample_data) -> None:
    formatted_data, data_list = sample_data
    bot = FormatData(formatted_data, data_list)
...
def test_search_output(category, expected, sample_data) -> None:
    bot = FormatData(*sample_data)
    result = bot.search(category)
    assert result == expected


def test_search_no_sport_match(sample_data) -> None:
    bot = FormatData(*sample_data)
    assert bot.search("غير موجود") == ""


def test_search_missing_label(sample_data) -> None:
    formatted_data, data_list = sample_data
    bot = FormatData({}, data_list)
    assert bot.search("men's football world cup") == ""


def test_search_missing_sport_label(sample_data) -> None:
    formatted_data, data_list = sample_data
...
# --- Case-insensitivity ----------------------------------------------
def test_case_insensitive_match(sample_data) -> None:
    bot = FormatData(*sample_data)
