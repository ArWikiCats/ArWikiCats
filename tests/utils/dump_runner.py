...
def _run_dump_case(name: str, data: dict[str, str], callback: callable, run_same=False) -> None:
    """
...
def make_dump_test_name_data(to_test: ToTest, callback, run_same=False):
    ...
    def test_dump_all(name: str, data: dict[str, str]) -> None:
        _run_dump_case(name, data, callback, run_same=run_same)

    return test_dump_all


def make_dump_test_name_data_callback(to_test: ToTestCallback, run_same=False):
    """
    Create a parametrized pytest test function.
    """
